...
agent_executor = initialize_chain(ConversationBufferMemory())
...
if user_input:
  ...
  result = agent_executor.run(user_input, tools=tools)
...
# Initialize chat_memory in session state    
...
if user_input:
  ...
  try:
    with st.container():
      ...
      #callbacks is what makes the thining code
      st.write(result)
...
# This will now include 'chat_memory'
